...
import com.zongyuheng.content.IndexContent;
...
@Component
public class IndexPageManager<E> {
    private int size= IndexContent.PAGE_SIZE;//容量
    private int totalPages;  //总页数
    private int totalElements;//总元素
    private int num;//第几页  前端传过来的
    private boolean first; //是否是第一页
    private String[] sort={IndexContent.SORT_METHOD, IndexContent.SORT_TARGET};//按照什么元素根据什么顺序进行排序
    ...
    public IndexPageManager(int size, int totalPages, int totalElements, int num, boolean first, String[] sort, int numberOfElements, List<E> results, boolean last) {
        this.size = size;
